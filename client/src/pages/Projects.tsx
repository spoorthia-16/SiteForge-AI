...
import ProjectPreview, { type ProjectPreviewRef } from '../components/ProjectPreview.tsx'
      ...
      <div className='flex max-sm:flex-col sm:items-center gap-4 px-4 py-2 no-scrollbar'>
        ...
        <div className='flex items-center gap-2 sm:min-w-90 text-nowrap'>
          ...
          <div className='sm:hidden flex-1 flex justify-end'>
            {isMenuOpen ? <MessageSquareIcon onClick={()=> setIsMenuOpen(false)} className='size-6 cursor-pointer'/> : <XIcon onClick={()=> setIsMenuOpen(false)} className='size-6 cursor-pointer'/>}
          </div>
        </div>
        {/* middle */}
        ...
        <div className='flex items-center justify-end gap-3 flex-1 text-xs sm:text-sm'>
          ...
          <Link target='_blank' to={`/preview/${projectId}`} className='flex items-center gap-2 px-4 py-1 rounded sm:rounded-sm border border-gray-700  hover:border-gray-500 transition-colors'> <FullscreenIcon size={16}/>Preview
          </Link>
